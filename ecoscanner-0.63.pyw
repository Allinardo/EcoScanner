# eco_recipe_gui_with_notes.pyw – v0.62 Merged
...
DEFAULT_ECO_PATH = "C:\\Program Files (x86)\\Steam\\steamapps\\common\\Eco"
LOG = "eco_parser.log"
...
# Add this custom tree widget class to your code (insert before the GUI class)

class CustomTreeWidget(tk.Frame):
    """Custom tree widget with virtual scrolling and background images"""
    
    def __init__(self, parent, bg_image_path, item_height=68, item_width=533, **kwargs):
        ...
        self.visible_items = {}  # Dict to track currently visible item widgets
        ...
        self.bg_image = None
        self.bg_photo = None
        if Path(bg_image_path).exists() and Image:
            try:
                self.bg_image = Image.open(bg_image_path)
                if self.bg_image.size != (item_width, item_height):
                    self.bg_image = self.bg_image.resize((item_width, item_height), Image.LANCZOS)
                self.bg_photo = ImageTk.PhotoImage(self.bg_image)
            except Exception as e:
                log(f"Error loading tree background image: {e}")
        ...
        self.canvas.bind("<Button-4>", lambda e: self._scroll_by(-3))
        self.canvas.bind("<Button-5>", lambda e: self._scroll_by(3))
    ...
    def _on_scroll(self, *args):
        ...
        if args[0] == 'moveto':
            # Calculate position based on total items
            total_height = len(self.items) * self.item_height
            canvas_height = self.canvas.winfo_height()
            if total_height > canvas_height:
                pos = float(args[1])
                self.viewport_start = int(pos * (total_height - canvas_height) / self.item_height)
            else:
                self.viewport_start = 0
        elif args[0] == 'scroll':
            self.viewport_start += int(args[1])
        
        # Clamp viewport start
        max_start = max(0, len(self.items) - (self.canvas.winfo_height() // self.item_height) - 1)
    ...
    def _on_mousewheel(self, event):
        ...
        self._scroll_by(int(-2 * (event.delta/120)))
        return "break"
    
    def _scroll_by(self, units):
        """Scroll by a number of items"""
        self.viewport_start += units
        max_start = max(0, len(self.items) - (self.canvas.winfo_height() // self.item_height) - 1)
        self.viewport_start = max(0, min(self.viewport_start, max_start))
        self._update_visible_items()
        self._update_scrollbar()
    ...
    def _update_scrollbar(self):
        ...
        if total_height <= canvas_height:
            ...
            first = self.viewport_start * self.item_height / total_height
            last = (self.viewport_start * self.item_height + canvas_height) / total_height
    ...
    def _update_visible_items(self):
        ...
        # If canvas hasn't been drawn yet, schedule for later
        if canvas_height <= 1:
            self.after(10, self._update_visible_items)
            return
        
        # Clear ALL canvas windows and recreate them
        ...
        # Calculate viewport
        items_visible = (canvas_height // self.item_height) + 2  # Add buffer
        self.viewport_end = min(self.viewport_start + items_visible, len(self.items))
        
        # Create visible items at correct positions
    ...
    def _create_item_widget(self, index):
        """Create a widget for an item at the given index"""
        ...
        # Calculate position
        y_pos = (index - self.viewport_start) * self.item_height
        
        # Create item frame
        item_frame = tk.Frame(self.canvas, height=self.item_height, width=self.item_width)
        item_frame.pack_propagate(False)
        
        # Create item canvas
        item_canvas = tk.Canvas(item_frame, highlightthickness=0, bd=0,
                               height=self.item_height, width=self.item_width)
        item_canvas.pack(fill="both", expand=True)
        
        # Add background
        if self.bg_photo:
            item_canvas.create_image(0, 0, anchor="nw", image=self.bg_photo)
        else:
            item_canvas.configure(bg='#3a3a3a')
        
        # Add icon
        icon_x = 10
        if item_data.get('icon'):
            try:
                self.item_photos[f"icon_{index}"] = item_data['icon']
                item_canvas.create_image(icon_x + 24, self.item_height // 2,
                                        anchor="center", image=item_data['icon'])
                icon_x += 60
            except:
                pass
        
        # Add text
        text_color = '#ffffff' if index != self.selected_index else '#463422'
        item_canvas.create_text(icon_x + 5, self.item_height // 2,
                               text=item_data['text'], anchor="w",
                               font=('LexusCapsOpti-Medium', 18), fill=text_color,
                               tags="item_text")
        
        # Bind events
        item_canvas.bind("<Button-1>", lambda e: self._select_item(index))
        item_canvas.bind("<MouseWheel>", self._on_mousewheel)
        
        # Place on canvas
        self.canvas.create_window(0, y_pos, anchor="nw", window=item_frame)
        self.visible_items[index] = item_frame
    ...
    def add_item(self, text, icon_image=None, data=None):
    ...
    def refresh_display(self):
        """Manually refresh the display after batch operations"""
        self._update_visible_items()
        self._update_scrollbar()
    ...
    def _select_item(self, index):
        ...
        # Update display for both old and new selection
        if old_selected is not None and old_selected in self.visible_items:
            self.visible_items[old_selected].destroy()
            del self.visible_items[old_selected]
            self._create_item_widget(old_selected)
        
        if index in self.visible_items:
            self.visible_items[index].destroy()
            del self.visible_items[index]
            self._create_item_widget(index)
        
        # Trigger callback
        if self.on_select_callback:
            self.on_select_callback(index, self.items[index])
    ...
    def see(self, index):
        ...
        canvas_height = self.canvas.winfo_height()
        items_visible = canvas_height // self.item_height
        
        if index < self.viewport_start:
            self.viewport_start = index
        elif index >= self.viewport_start + items_visible:
            self.viewport_start = index - items_visible + 1
    ...
    def bind_select(self, callback):
        """Bind a callback for selection events"""
...
class GUI:
    def __init__(s, root):
        s.root = root
        s.r = root
        s.root.geometry("1600x900")  # Set initial window size
        
        # Setup window icon and title
        setup_window_icon_and_title(root)
        
        # Center the window on screen
        s._center_window(1600, 900)
    ...
    def _add_tiled_background(s, parent_frame, image, photo, header_text=""):
        """Add a tiled background to a frame using a canvas with optional header text"""
        # Create the canvas that will hold the tiled background
        canvas = tk.Canvas(parent_frame, highlightthickness=0, bd=0)
        
        # Place the canvas to fill the entire frame
        # Since it's created first, it will be at the bottom layer
        canvas.place(relx=0, rely=0, relwidth=1, relheight=1)
        
        def tile_background(event=None):
            """Function to tile the background image across the canvas"""
            canvas.delete("all")  # Clear any existing tiles and text
            ...
            # Only tile if we have valid dimensions and image
            if image and photo and w > 1 and h > 1:
                # Calculate how many tiles we need
                tiles_x = (w // image.width) + 2  # Add extra tiles for safety
                tiles_y = (h // image.height) + 2
                ...
                # Draw the header text if provided
                if header_text:
        ...
        # Bind the tiling function to resize events
        parent_frame.bind("<Configure>", tile_background)
        
        # Schedule initial tiling after the frame has been properly rendered
        parent_frame.after(50, tile_background)
        
        # Also trigger tiling when the canvas itself is configured
        canvas.bind("<Configure>", lambda e: parent_frame.after(10, tile_background))
        
        # No lower() call needed - proper widget ordering handles stacking
    ...
    def _setup_fonts(s):
        ...
        lexus_path = fonts_dir / 'LexusCapsOpti-Medium.otf'
    ...
    def _create_ui(s):
        ...
        s.main_frame = s.r
        # --- Dual-layer tiled background with BG2 bleed 8px and BG1 inset 8px ---
        s.bg_canvas = tk.Canvas(s.main_frame, highlightthickness=0, bd=0)
        s.bg_canvas.place(relx=0, rely=0, relwidth=1, relheight=1)
        
        # Ensure PIL is available
        if Image is None or ImageTk is None:
            print('[WARN] Pillow not available; background disabled')
        else:
            s.bg1_image = Image.open('UI/BG1.png')
            s.bg1_tile = ImageTk.PhotoImage(s.bg1_image)
            s.bg2_image = Image.open('UI/BG2.png')
            s.bg2_tile = ImageTk.PhotoImage(s.bg2_image)

            # Load Scan_BG texture for section headers
            s.scan_bg_image = None
            s.scan_bg_photo = None
            scan_bg_path = script_dir / 'UI' / 'Scan_BG.png'
            if scan_bg_path.exists() and Image:
                try:
                    s.scan_bg_image = Image.open(scan_bg_path)
                    s.scan_bg_photo = ImageTk.PhotoImage(s.scan_bg_image)
                except Exception as e:
                    log(f"Error loading Scan_BG.png: {e}")
            
            def _tile_layer(canvas, img, photo, x0, y0, w, h, tag):
                ...
                while y < end_y:
                    x = start_x
                    while x < end_x:
                        canvas.create_image(x, y, image=photo, anchor='nw', tags=('bg_tile', tag))
                        x += iw
                    y += ih

            def draw_tiled_bg(event=None):
                c = s.bg_canvas
                c.delete('bg_tile')
                w = s.main_frame.winfo_width()
                h = s.main_frame.winfo_height()
                if w <= 0 or h <= 0:
                    return
                bleed = 8
                _tile_layer(c, s.bg2_image, s.bg2_tile, -bleed, -bleed, w + bleed*2, h + bleed*2, 'bg2')
                inset = 8
                inner_w = max(0, w - inset*2)
                inner_h = max(0, h - inset*2)
                _tile_layer(c, s.bg1_image, s.bg1_tile, inset, inset, inner_w, inner_h, 'bg1')

            s.main_frame.bind('<Configure>', draw_tiled_bg)
            # Lower the canvas widget (not items) after UI is created
            s.r.after(0, lambda: s.bg_canvas.lower())
        # --- end background block ---

        
        # Top section - control bar with parsing and folder selection
        top = tk.Frame(s.main_frame, bg='', highlightthickness=0, bd=0, height=100)  # Increased from 80 to 100
        top.pack(fill="x", padx=6, pady=(10,0))
        top.pack_propagate(False)

        # Create a container for absolute positioning in top-left
        top_left_container = tk.Frame(top, bg='', highlightthickness=0, bd=0)
        top_left_container.place(x=0, y=0)  # Place at absolute top-left of the top frame

        # Create a vertical frame for the buttons
        button_frame = tk.Frame(top_left_container, bg='', highlightthickness=0, bd=0)
        button_frame.pack(side="left", padx=(10, 15))  # Small left padding to not be flush against edge

        # Parse button with image
        script_dir = Path(__file__).parent
        parse_img_path = script_dir / 'UI' / 'Parse.png'

        if parse_img_path.exists() and Image:
            try:
                parse_img = Image.open(parse_img_path)
                s.parse_photo = ImageTk.PhotoImage(parse_img)
                parse_btn = tk.Button(button_frame, image=s.parse_photo, command=s.run,
                                    bd=0, highlightthickness=0, relief='flat')
                parse_btn.image = s.parse_photo
                parse_btn.pack(pady=(0, 5))
            except Exception as e:
                log(f"Error loading Parse.png: {e}")
                parse_btn = tk.Button(button_frame, text="PARSE", command=s.run, 
                                    font=s.ui_font_bold, bg='#4CAF50', fg='white',
                                    padx=20, pady=10)
                parse_btn.pack(pady=(0, 5))
        else:
            parse_btn = tk.Button(button_frame, text="PARSE", command=s.run, 
        ...
        # Folder button with image
        folder_img_path = script_dir / 'UI' / 'FolderLocation.png'

        if folder_img_path.exists() and Image:
            try:
                folder_img = Image.open(folder_img_path)
                s.folder_photo = ImageTk.PhotoImage(folder_img)
                folder_btn = tk.Button(button_frame, image=s.folder_photo, command=s.pick,
                                    bd=0, highlightthickness=0, height=30, relief='flat')
                folder_btn.image = s.folder_photo
                folder_btn.pack(anchor='w', padx=(0, 10))
            except Exception as e:
                log(f"Error loading FolderLocation.png: {e}")
                tk.Button(button_frame, text="Eco Game Folder...", command=s.pick, font=s.ui_font).pack()
        else:
            tk.Button(button_frame, text="Eco Game Folder...", command=s.pick, font=s.ui_font).pack()

        # Path Entry and status controls in a separate frame positioned to the right
        path_frame = tk.Frame(top, bg='', highlightthickness=0, bd=0)
        path_frame.place(x=180, y=68)  # Position to the right of buttons

        tk.Entry(path_frame, textvariable=s.folder, width=50, font=s.ui_font).pack(side="left", padx=4)

        # Status display
        s.st = tk.Label(path_frame, fg="blue", font=s.ui_font)
        s.st.pack(side="left", padx=8)
        s.pb = ttk.Progressbar(path_frame, mode="indeterminate", length=140)
        ...
        s.tags_filter_btn.pack(side="left", padx=(0, 5))

        # ADD THIS LINE - Create the filter label that shows current active filter
        s.current_filter_label = tk.Label(sr, text="", font=s.ui_font, fg="blue")
        s.current_filter_label.pack(side="left", padx=(5, 10))

        # Search field (last)
        tk.Label(sr, text="Search:", font=s.ui_font).pack(side="left", padx=(20, 0))
        ...
        # Tree view on left (item list) - Custom widget with background
        tree_frame = tk.Frame(pane)

        # Use custom tree widget instead of ttk.Treeview
        script_dir = Path(__file__).parent
        tree_bg_path = script_dir / 'UI' / 'UI_Tree.png'

        s.tree = CustomTreeWidget(tree_frame, 
                                bg_image_path=str(tree_bg_path),
                                item_height=68,
                                item_width=533)
        s.tree.pack(fill="both", expand=True)


        # Bind selection callback
        def on_tree_select(index, item_data):
            # Store the selected index for compatibility
            s.tree.selected_index = index
            # Call the existing show method
            s.show()

        s.tree.bind_select(on_tree_select)

        # Bind keyboard events to the tree's canvas for key navigation
        s.tree.canvas.bind("<KeyPress>", s._on_tree_key_press)
        ...
        visibility_label = tk.Label(s.admin_tools_frame, text="Item Visibility", font=s.ui_font_bold, fg='red', bg='#ffe6e6')
        visibility_label.pack(anchor='w', padx=2, pady=(10,2))

        visibility_frame = tk.Frame(s.admin_tools_frame, bg='#ffe6e6')
        visibility_frame.pack(fill="x", pady=2, padx=2)

        s.hide_item_btn = tk.Button(visibility_frame, text="Hide Current Item", 
                                    command=s.hide_current_item, font=s.ui_font, 
                                    bg='#FFA500', fg='white', state='disabled')
        s.hide_item_btn.pack(fill="x", pady=2)

        tk.Button(visibility_frame, text="Manage Hidden Items", 
                command=s.show_hidden_items, font=s.ui_font, 
                bg='#FFD700').pack(fill="x", pady=2)
        ...
        desc_frame = tk.Frame(sections_container)
        desc_frame.pack(fill="both", expand=False, pady=(0, 2))
        
        desc_header = tk.Frame(desc_frame, height=30)  # Set explicit height
        desc_header.pack(fill="x", padx=4, pady=2)
        desc_header.pack_propagate(False)  # Prevent frame from shrinking

        # Add tiled background if available
        if s.scan_bg_image and s.scan_bg_photo:
            s._add_tiled_background(desc_header, s.scan_bg_image, s.scan_bg_photo, "Item Description")
        ...
        s.desc_txt = tk.Text(desc_text_frame, wrap="word", font=s.main_text_font, 
                    height=14, yscrollcommand=desc_scroll.set, state="disabled",
        ...
        recipe_frame = tk.Frame(sections_container)
        recipe_frame.pack(fill="both", expand=False, pady=(2, 2))

        # Create a container for the header and navigation controls
        recipe_top_container = tk.Frame(recipe_frame)
        recipe_top_container.pack(fill="x", padx=4, pady=2)

        # Left side: Recipe header with tiled background
        recipe_header = tk.Frame(recipe_top_container, height=30)
        recipe_header.pack(side="left", fill="x", expand=True)
        recipe_header.pack_propagate(False)

        # Add tiled background with "Recipe Information" text
        if s.scan_bg_image and s.scan_bg_photo:
            s._add_tiled_background(recipe_header, s.scan_bg_image, s.scan_bg_photo, "Recipe Information")

        # Right side: Navigation controls (OUTSIDE the tiled area)
        recipe_nav_frame = tk.Frame(recipe_top_container)
        recipe_nav_frame.pack(side="right", padx=(10, 0))
        ...
        # Now create the text frame for recipe content
        ...
        used_in_frame = tk.Frame(sections_container)
        used_in_frame.pack(fill="x", expand=False, pady=(2, 2))
        
        used_in_header = tk.Frame(used_in_frame, height=30)  # Set explicit height
        used_in_header.pack(fill="x", padx=4, pady=2)
        used_in_header.pack_propagate(False)  # Prevent frame from shrinking

        # Add tiled background if available
        if s.scan_bg_image and s.scan_bg_photo:
            s._add_tiled_background(used_in_header, s.scan_bg_image, s.scan_bg_photo, "Used In")
        ...
        pane.add(content_pane, weight=5)
            ...
            for icon_name in possible_names:
                ...
                if icon_path.exists() and Image:
                    try:
                        ...
                        im = Image.open(icon_path).resize((60, 60), Image.LANCZOS)
    ...
    def run(s):
        ...
        s.pb.start()
        s.tree.clear()  # Use the custom tree widget's clear method
        s.icons.clear()
        s.data.clear()
    ...
    def filter(s, *_):
        q = s.q.get().lower()
        s.tree.clear()  # Clear custom tree widget
        s.filtered_data = []

        # Debug: Print first and last 5 items
        print(f"First 5 items: {[item['result_item'] for item in s.filtered_data[:5]]}")
        print(f"Last 5 items: {[item['result_item'] for item in s.filtered_data[-5:]]}")
        print(f"Total items to display: {len(s.filtered_data)}")
        ...
        # Add all items first without updating
        for filtered_idx, rec in enumerate(s.filtered_data):
            ...
            # Add item to custom tree (remove the leading space from display_text)
            display_text = display_text.strip()  # Remove leading space
            s.tree.add_item(display_text, icon_img, data=rec)

        # Now update the display only once after all items are added
        s.tree.refresh_display()
    ...
    def hide_current_item(s):
        """Hide the currently selected item (admin function)"""
        if not s.selected_item:
            messagebox.showwarning("No Selection", "Please select an item to hide.")
            return
        
        # Double confirmation since it's in admin tools
        result = messagebox.askyesno(
            "Hide Item - Admin Function", 
            f"Are you sure you want to hide '{s.selected_item}' from the list?\n\n"
            "This will remove it from the main view.\n"
            "You can restore it later from 'Manage Hidden Items'."
        )
        
        if result:
            s._hidden_items.add(s.selected_item)
            s._save_hidden_items()
            s.filter()
            s.st.config(text=f"Hidden '{s.selected_item}' (Admin action)")
            
            # Clear the selection since the item is now hidden
            s.selected_item = None
            ...
            s.used_in_txt.config(state='normal')
            s.hide_item_btn.config(state='normal')
    ...
    def show(s, *_):
        sel_index = s.tree.get_selection()
        if sel_index is None: 
            return
        rec = s.filtered_data[sel_index]
        name = rec['result_item']
        ...
        # Enable/disable hide button based on admin tools visibility and selection
        if hasattr(s, 'hide_item_btn'):
            if s.admin_tools_visible and s.selected_item:
                s.hide_item_btn.config(state='normal')
            else:
                s.hide_item_btn.config(state='disabled')
    ...
    def _build_separated_display(s, rec):
        print(f"DEBUG: recipe_txt has {len(s.recipe_txt.winfo_children())} child widgets before clearing")
            ...
            if rec['result_item'].lower() == item_name.lower():
                ...
                for filtered_idx, filtered_rec in enumerate(s.filtered_data):
                    if filtered_rec['result_item'] == rec['result_item']:
                        s.tree.set_selection(filtered_idx)
                        s.show()
                        found = True
                        break
                break
    ...
    def _handle_ingredient_click(s, ingredient_name):
        ...
        if not is_tag:
            ...
            # Check both the original name and CamelCase version
            if ingredient_name in all_tags or spaced_to_camel_case(ingredient_name) in all_tags:
                is_tag = True
    ...
    def _update_filter_label_with_icon(s, tag_name):
        ...
        # Try to load the tag icon - try multiple naming conventions
        script_dir = Path(__file__).parent
        icon_found = False

        possible_names = [
            tag_name,                          # Try exact name first (e.g., "Food Item")
            spaced_to_camel_case(tag_name),   # Try CamelCase (e.g., "FoodItem")
            tag_name.replace(" ", "_"),        # Try with underscores (e.g., "Food_Item")
        ]

        for tag_icon_name in possible_names:
            tag_icon_path = script_dir / 'TagIcons' / f"{tag_icon_name}.png"
    ...
    def _update_tags_display(s, item):
        ...
        for tag_idx, tag in enumerate(tags):
            ...
            # Try multiple naming conventions for tag icon lookup
            possible_names = [
                tag,                          # Try exact name first
                spaced_to_camel_case(tag),   # Try CamelCase
                tag.replace(" ", "_"),        # Try with underscores
            ]
...
if __name__ == "__main__":
    try:
        log("Starting Eco Recipe Scanner v0.61 - Merged Version...")
        root = tk.Tk()
        GUI(root)
        root.mainloop()
    except Exception as e:
